...
class Second_Level_Trainer:
    ...
    def loop(self, env, agents, database, n_episodes=10, train=True,
        ...
        for episode in range(0, n_episodes):
            ...
            if train and ((episode+1) % eval_each == 0):
                eval_returns = self.loop(env, agents, None, n_episodes=5, train=False, 
                    max_episode_steps=max_episode_steps, update_database=False,
                    render=False, store_video=False, wandb_project=wandb_project,
                    save_model=False, greedy_sampling=True, initialization=False,
                    n_step_td=1)
                wandb.log({'episode_eval': episode//eval_each, 'eval_return': eval_returns.mean(), 'eval_return_std':eval_returns.std()})
...
class Third_Level_Trainer:
    ...
    def loop(self, env, agents, database, n_episodes=10, train=True,
            ...
            save_model=True, save_model_each=50, MODEL_PATH='', 
            save_step_each=2, greedy_sampling=False, initialization=True,
            init_buffer_size=500, n_step_td=2, eval_each=10, train_n_MC=2,
            rest_n_MC=1, eval_MC=False, dual_eval=True):
        ...
        for episode in range(0, n_episodes):
            ...
            if train and ((episode+1) % eval_each == 0):
                ...
                if dual_eval:
                    eval_returns_MC = self.loop(env, agents, None, n_episodes=10, train=False, 
                        max_episode_steps=max_episode_steps, update_database=False,
                        render=False, store_video=False, wandb_project=wandb_project,
                        save_model=False, greedy_sampling=True, initialization=False,
            ...
            if train_MC_episode:
                ...
                high_level_metrics = self.optimizer.optimize_tabular(agents[-1], trajectory_buffer, last_MC_episode)
