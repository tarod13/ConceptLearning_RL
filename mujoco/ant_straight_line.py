...
class AntStraightLineEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,
                 xml_file='ant_v2.xml',
                 ctrl_cost_weight=0.0, #0.5
                 contact_cost_weight=0.0, #5e-4
    ...
    def get_current_maze_obs(self):   
        position = np.asarray(self.get_body_com("torso")[:3], dtype=np.float64)   
        ...
        for ray in range(self._n_rays):
            ...
            distance, obstacle_id = self.sim.ray_fast_group(position, ray_orientation, self._obstacle_filter)           
